...
export default async function InstitutionRequestPage() {
  ...
  // 학교요청전달여부(institution_request_delivered)는 이제 날짜별(event_dates, 그룹 있으면
  // event_groups) 값이라 events에서 직접 필터링하지 않고, 아래에서 날짜별로 펼친 뒤 필터링한다.
  ...
  const rows: InstitutionRequestRow[] = []
  for (const e of events) {
    ...
    for (const date of dates) {
      ...
      if (resolved?.institutionRequestDelivered) continue
