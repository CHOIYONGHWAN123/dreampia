...
export default async function PhotoDeliveryPage() {
  ...
  // 영업담당자 또는 소통담당자로 지정된 행사 중, 아직 행사 사진을 전달하지 않은 것만 표시.
  // 슈퍼관리자는 담당자 여부와 무관하게 전체 조회.
  let eventsQuery = supabase
    .from('events')
    .select('id, institution_id, sales_admin_id, comm_admin_id, event_start_at, event_end_at, photo_sent')
    .eq('photo_sent', false)
  if (!isSuper) {
  ...
  const rows: PhotoDeliveryRow[] = activeEvents.map((e, i) => ({
    no: i + 1,
    id: e.id,
