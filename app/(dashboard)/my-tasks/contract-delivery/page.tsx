...
import { ContractDeliveryClient, type ContractDeliveryRow } from '@/components/features/my-tasks/ContractDeliveryClient'

export default async function ContractDeliveryPage() {
  ...
  if (!events || events.length === 0) {
    return <ContractDeliveryClient rows={[]} />
  }
  ...
  const rows: ContractDeliveryRow[] = activeEvents.map((e, i) => ({
    no: i + 1,
    id: e.id,
  ...
  return <ContractDeliveryClient rows={rows} />
}
