...
import { AdminDocsDeliveryClient, type AdminDocsDeliveryRow } from '@/components/features/my-tasks/AdminDocsDeliveryClient'
...
export default async function AdminDocsDeliveryPage() {
  ...
  // 아직 행정서류를 전달하지 않은 것만 표시.
  const { data: events } = await supabase
    .from('events')
    .select('id, institution_id, sales_admin_id, comm_admin_id, event_start_at, event_end_at, admin_docs_delivered')
    .or(`sales_admin_id.eq.${user.id},comm_admin_id.eq.${user.id}`)
    .eq('admin_docs_delivered', false)
    .order('event_start_at', { ascending: false, nullsFirst: false })

  if (!events || events.length === 0) {
    return <AdminDocsDeliveryClient rows={[]} />
  }
  ...
  const rows: AdminDocsDeliveryRow[] = activeEvents.map((e, i) => ({
    no: i + 1,
    id: e.id,
  ...
  return <AdminDocsDeliveryClient rows={rows} />
}
