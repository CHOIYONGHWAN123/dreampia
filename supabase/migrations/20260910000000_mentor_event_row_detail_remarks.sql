-- 멘토 앱 강의 상세 화면에 event_rows.remarks("특이사항")가 아예 빠져 있어 클라이언트가
-- 값을 입력해도 강사에게 보이지 않는 문제를 확인했다. 현장운영자용 조회
-- (get_field_operator_event_rows/get_field_operator_event_detail)에는 이미 remarks가
-- 포함되어 있는데, 강사 본인용 mentor_event_row_detail 뷰/get_sub_mentor_event_row_detail
-- 함수에서만 누락되어 있었다. 뷰는 끝에 컬럼을 추가하는 것만 CREATE OR REPLACE로 허용되고,
-- 함수는 반환 타입이 바뀌므로 DROP 후 다시 만들어야 한다.
