    ...
    try {
      ...
      if (user) {
        const { error: logErr } = await supabase
          .from('work_logs')
          .insert({ admin_id: user.id, event_id: eventId, task_type: '행사 사진 전달' })
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between pb-4 border-b border-gray-200 mb-6">
        <h1 className="text-2xl font-bold text-gray-900">행사 사진 전달</h1>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-amber-50 border-b border-gray-200">
              ...
              <th className="px-4 py-2.5 text-center font-medium text-gray-700 w-32">행사 사진 전달 완료</th>
            </tr>
          </thead>
          <tbody>
