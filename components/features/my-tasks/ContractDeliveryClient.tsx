...
export function ContractDeliveryClient({ rows }: { rows: ContractDeliveryRow[] }) {
  ...
  const handleMarkDelivered = async (eventId: string) => {
    if (!confirm('계약 전달을 완료하셨습니까?')) return

    setUpdatingId(eventId)
    try {
      const { error } = await supabase
        .from('events')
        .update({ contract_delivered: true })
        .eq('id', eventId)
      ...
      if (user) {
        const { error: logErr } = await supabase
          .from('work_logs')
          .insert({ admin_id: user.id, event_id: eventId, task_type: '계약 전달' })
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-full">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">계약 전달</h1>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr>
              ...
              <th className="sticky top-0 z-10 bg-primary-50 border-b border-primary-100 px-4 py-2.5 text-center font-bold text-primary-700 w-32">계약 전달 완료</th>
            </tr>
          </thead>
          <tbody>
