...
export function PreNoticeClient({ rows }: { rows: PreNoticeRow[] }) {
  ...
  const handleMarkSent = async (row: PreNoticeRow) => {
    if (!confirm('행사 안내를 완료하셨습니까?')) return

    setUpdatingId(row.id + row.dateKey)
    try {
      if (row.groupId) {
        await updateEventGroupField(row.groupId, { pre_notice_sent: true })
      } else {
        await updateEventDateField(row.id, row.dateKey, { pre_notice_sent: true })
      }
      ...
      if (user) {
        const { error: logErr } = await supabase
          .from('work_logs')
          .insert({ admin_id: user.id, event_id: row.id, task_type: '행사 안내' })
              ...
              filteredRows.map((row) => (
                <tr key={row.id + row.dateKey} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-center">
                    ...
                    >
                      {updatingId === row.id + row.dateKey ? '처리중...' : '안내 완료'}
                    </button>
                  </td>
                </tr>
              ))
